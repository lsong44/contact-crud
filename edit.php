...
isset($_POST['profile_id']))
{
  ...
  || !$_POST['source'] || !$_POST['state'] || $_POST['transferred'] ==""){
    ...
    $sql= "UPDATE Profile SET first_name = :first_name, last_name = :last_name,
            title = :title, edited_time = :added_time, transferred_backup = :transferred,
            source_id = :src_id, state_id =:st_id WHERE profile_id = :pid;";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
    ...
    //delete from email
    $sql = "DELETE FROM Email WHERE profile_id = :pid";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(':pid' => $pf_id));
    //insert into email
    for($i=1; $i<=10; $i++){
    ...
    //delete from phone
    $sql = "DELETE FROM Phone WHERE profile_id = :pid";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(':pid' => $pf_id));
    //insert into phone
    for($i=1; $i<=10; $i++){
    ...
    //delete from address
    $sql = "DELETE FROM Address WHERE profile_id = :pid";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(':pid' => $pf_id));
    //insert into address
    for($i=1; $i<=10; $i++){
...
//show exsiting values in the form
$sql = "SELECT * FROM Profile p LEFT JOIN Address a ON p.profile_id = a.profile_id
        LEFT JOIN state st ON st.state_id = p.state_id
        LEFT JOIN source sr ON sr.source_id = p.source_id
        LEFT JOIN Email e ON p.profile_id = e.profile_id
        LEFT JOIN Phone ph ON p.profile_id = ph.profile_id
        LEFT JOIN internal_user i ON p.user_id = i.user_id
        WHERE p.profile_id = :id";
$stmt = $pdo->prepare($sql);
$stmt->execute(array(':id' => $_GET['profile_id']));
$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
if(!$rows){
  $_SESSION['error'] = "Cannot find the record";
  header("Location: view.php?user=".$_SESSION['username']);
  return;
}
  ...
  <div class="container">
    <div class="row" style="margin-top: 50px">
      <div calss="page-header">
        <h1>Edit the profile for <?= $rows[0]['first_name']." ".$rows[0]['last_name'] ?></h1>
      </div>
    </div>
    <div class="row" style="margin-top: 50px">
      <?php
      ...
      <form method="post">
        <fieldset>
          <div class="form-group">
            ...
            id="first_name" value = <?= $rows[0]['first_name'] ?> required>
          </div>
          <div class="form-group">
            <label for="last_name">Last Name</label>
            <input type="text" name = "last_name" class="form-control"
            id="last_name" value=<?= $rows[0]['last_name'] ?> required>
          </div>
          <div class="form-group">
            <label for="title">Title</label>
            <input type="text" name = "title" class="form-control"
            id="title" value=<?= $rows[0]['title'] ?> required>
          </div>
          <div class="form-group">
            <label for="state">State</label>
            <input type="text" name = "state" class="form-control state ui-autocomplete-input"
              autocomplete="off" id="state" value=<?= $rows[0]['state_name'] ?> required>
          </div>
          <div class="form-group row">
            <label for="add-address" class="col-sm-9 col-form-label">Address</label>
            <div class="col-sm-3">
          ...
          <div id="address-fields">
            <?php
              ...
              foreach($rows as $row){
                if($row['city']){
                  $countAdd++;
                  echo('<div id="address'.$countAdd.'" name="address '.$countAdd.'">'."\n");
                  echo('<div class="form-group">'."\n");
                  echo('<div class="form-group row">'."\n");
                  echo('<label class="col-sm-9 col-form-label" for="street'.$countAdd.'">
                      Street Address '.$countAdd."</label>\n");
                  echo("<div class='col-sm-3'>\n");
                  echo('<input type="submit" class="btn btn-primary btn-sm" value="-"
                        onclick = "$(\'#address'.$countAdd.'\').remove(); return false;" formnovalidate/>
                      </div></div>'."\n");
                  echo('<input type="text" name = "street'.$countAdd.'" class="form-control"
                    value='.$row["street"].' id="street'.$countAdd.'"></div>'."\n");
                  echo('<div class="form-group">');
                  echo('<label for="city'.$countAdd.'">City</label>
                    <input type="text" name = "city'.$countAdd.'" class="form-control"
                    id="city'.$countAdd.'" value='.$row['city'].'></div>'."\n");
                  echo('<div class="form-group">
                    <label for="zip_code'.$countAdd.'">Zip Code</label>
                    <input type="text" name = "zip_code'.$countAdd.'" class="form-control"
                    id="zip_code'.$countAdd.'" value='.$row['zip_code'].'>  </div></div>'."\n");
                }
              }
            ?>
          </div>
              ...
              foreach($rows as $row){
                if($row['email']){
                  ...
                  <div class="form-group">
                    ...
                    id="email'.$countEmail.'" value='.$row['email'].'>
                  </div></div>');
                }
              }
            ?>
              ...
              foreach($rows as $row){
                if($row['phone']){
                  ...
                  <div class="form-group">
                    ...
                    id="phone'.$countPhone.'" value='.$row['phone'].'>
                  </div></div>');
                }
              }
            ?>
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="transferred" name="transferred" required>
              ...
              <option <?php if($rows[0]['transferred_backup']==1) echo("selected");?> value=1>Yes</option>
              <option <?php if($rows[0]['transferred_backup']==0) echo("selected");?> value=0>No</option>
            </select>
          </div>
          <div class="form-group">
            <label for="source">Source</label>
            <input type="text" name = "source" class="form-control source ui-autocomplete-input"
              autocomplete="off" id="source" value=<?= $rows[0]['source_name']?>>
          </div>
          <div class="form-group">
